...
| Runtime architecture | Implemented under ADR-0002 through ADR-0016 |
...
- A single person claims a temporary controller with one raised hand; in a multiperson frame, one person must raise both. Main Menu uses a frozen semantic row above the controlling head in portrait and a compact left column in landscape. Games, Draw, actionable Bubbles phases, and actionable Racing phases use a compact left column in either supported layout. Every action surface requires the coarse hand to leave its targets once before deliberate dwell activation arms, and every camera epoch releases temporal control state.
...
- Racing owns an ephemeral deterministic segmented course, pure pseudo-3D projection, three-second fresh-input wheel calibration, `5°` dead zone, full steering at `28°`, `80 ms` steering response, `150 ms` observation grace, automatic throttle, curve drift, off-road drag, and a bounded `60 Hz` fixed-step simulation. One-player accepts either layout and reports active elapsed time; two-player requires landscape and uses left/right split screen with bounded local torso leases. A one-second two-hands-overhead hold pauses once. The avatar is absent; each car view has a restrained wheel/tracking gauge. One lazy `phaser@4.2.1` runtime is forced to Canvas and fully destroyed on exit, with no WebGL or renderer fallback.
...
Run the one-player Lite baseline in [Pose quality](../engineering/pose-quality.md), then validate the complete journey on the project owner's real phone and television, including portrait, both landscape directions, reported orientation/source metadata, preview alignment, horizontal TV mirroring, stable epoch transitions, achieved camera-paced inference cadence, avatar rest stability and fast-motion lag, portrait/landscape menu transitions, two-player Bubbles/Racing gating, active Draw/Bubbles/Racing rotation recovery, Draw grip/path response, Bubbles hit tolerance/edge containment/side attribution, Racing load time, sustained one-/two-player Canvas cadence, calibration comfort, steering latency/stability, dropout feedback, overhead-pause false positives, split-screen readability, cleanup, and thermals. This external acceptance step must drive any orientation, threshold, model, collision, steering, course, or presentation replacement; it does not justify retaining alternate runtime paths.
...
- Default one-person detection, acknowledged one-/two-player switching, achieved inference cadence, processing-age p95, both landscape rotations, horizontal mirrored avatar presentation, avatar rest stability/fast-motion lag, portrait overhead framing, compact game-menu reach, stationary coarse-hand spread, temporary controller claiming, transition re-arming, Draw grip/path continuity, Bubbles gating/timing/hit tolerance/edge containment/side attribution, Racing calibration/steering/pause/split-screen behavior, per-renderer stability, thermal behavior, and perceived latency are recorded from the target hardware.

## Verification evidence

On 2026-08-15, `npm run validate` passed under Node 24.19.0/npm 11.17.0 after Racing integration: exact toolchain verification, formatting, linting, 153 unit/component tests, vendored-model integrity, two production builds, six Chromium end-to-end journeys, and the high-severity dependency audit all passed; npm reported zero vulnerabilities. The browser suite proved that the Racing chunk remains absent from initial startup, then loads the real two-player forced-Canvas Phaser runtime, renders pixels, emits snapshots, and removes its canvas on teardown. The separate Racing chunk measured `1,384.20 kB` minified / `361.27 kB` gzip and retains Vite's visible large-chunk advisory pending target-TV measurement.
...
| Racing wheel thresholds and temporary driver leases are unmeasured | Steering may feel noisy or laggy, overhead pause may false-trigger, or large movement/crossing may release a car slot | Record calibration, steering, dropout, pause, and two-player side behavior; change constants or lease rules only from target evidence and never add stable identity implicitly |
