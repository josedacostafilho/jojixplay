...
| Field | Current truth |
| --- | --- |
| Lifecycle | Greenfield / three-game prototype implemented / pre-release |
| Product purpose | Free phone-as-body-controller experiences; current proof is a live procedural body avatar plus Draw, Bubbles, and Racing |
| Application code | Phone-to-television body-control vertical slice and three games implemented |
| Runtime architecture | Implemented under ADR-0002 through ADR-0017 |
...
Run the one-player Lite baseline in [Pose quality](../engineering/pose-quality.md), then validate the complete journey on the project owner's real phone and television, including portrait, both landscape directions, reported orientation/source metadata, preview alignment, horizontal TV mirroring, stable epoch transitions, achieved camera-paced inference cadence, avatar rest stability and fast-motion lag, portrait/landscape menu transitions, two-player Bubbles/Racing gating, active Draw/Bubbles/Racing rotation recovery, Draw grip/path response, Bubbles hit tolerance/edge containment/side attribution, unobscured actionable message layouts, Racing load time, sustained one-/two-player Canvas cadence, natural-stance calibration comfort, `8°` enter/`3°` release behavior, steering latency/stability, dropout feedback, continuous road coverage at the car, overhead-pause false positives, split-screen readability, cleanup, and thermals. This external acceptance step must drive any orientation, threshold, model, collision, steering, course, or presentation replacement; it does not justify retaining alternate runtime paths.
...
On 2026-08-15, `npm run validate` passed under Node 24.19.0/npm 11.17.0 after the torso-lean/near-road hard cut: exact toolchain verification, formatting, linting, 154 unit/component tests, vendored-model integrity, two production builds, seven Chromium end-to-end journeys, and the high-severity dependency audit all passed; npm reported zero vulnerabilities. The browser suite proved that actionable Bubbles/Racing panels remain high and below controls, the Racing chunk remains absent from initial startup, then loads the real two-player forced-Canvas Phaser runtime, paints non-sky pixels at a formerly uncovered segment-boundary bottom edge, emits snapshots, and removes its canvas on teardown. The separate Racing chunk measured `1,384.64 kB` minified / `361.43 kB` gzip and retains Vite's visible large-chunk advisory pending target-TV measurement.
