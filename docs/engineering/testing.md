...
- Manual acceptance on a real phone and television is required before the hardware-dependent milestone is complete. It must cover portrait, both landscape directions, preview alignment, horizontal television mirroring, layout requests, active-game pause/resume, Racing startup and sustained one-/two-player Canvas cadence, torso-lean enter/release comfort and latency, tracking-loss feedback, and split-screen readability; automation cannot prove device camera metadata, body-control ergonomics, or target-TV performance.
