...
- Unit tests prove pairing-key entropy and normalization, strict fragment parsing, stable domain-separated credential derivation, pose-packet and player-limit validation, sequence rejection, opposite-role peer authentication, obsolete-protocol rejection, request/ack matching, request authorization, in-place worker reconfiguration, serialized inference/configuration, malformed-packet termination, extra-peer isolation, mirrored aspect-ratio mapping, latest-only send coalescing, controller-claim timing, multiperson contention, above-head geometry and rejection, frozen targets, coarse-hand projection/dropout, neutral arming, dwell latching, and every lease-release path.
- Component tests prove role selection, blocking phone-link errors, unsupported-device behavior, explicit headroom and arming guidance, semantic pose-action controls, acknowledged dynamic player labels, pending-action suspension, failure announcements, and the retained background/circle effects through accessible queries.
