...
The phone continues to infer pose and produces only validated canonical landmarks. Phaser and the complete race simulation exist only in the mounted shared playfield: on the same phone. No score, identifier, input history, asset state, or camera pixel is transmitted or persisted.
...
- Entering Racing captures the applied player count and landscape camera frame. Player count cannot change until Exit.
...
Both player counts use landscape. One player gets a full chase view; two players get left/right split views. Portrait unmounts the run and clears the game; no layout negotiation or restoration exists.
...
- Leases never leave playfield memory and reset on camera epoch, Restart, Play Again, Exit, local stop, or runtime destruction. They are not stable identities. Players should remain generally on their starting sides.
...
- Ready, pause, error, and result information remains semantic DOM above the canvas. Actionable message panels sit near the top center and below the button layer, so they never obscure or visually cover reachable controls. Active driving has no pose cursor or ordinary target buttons.
...
- Stale pose, camera epoch, portrait teardown, user pause, viewport resize, page suspension, local stop, and unmount have explicit reset or freeze semantics and create no steering bridge.
...
- Racing does not alter `PosePacket`, inference cadence, camera reconfiguration semantics, static deployment, or the shared camera runtime.
...
- [x] Add landscape Racing entry and hard-cut Games to a four-action compact left column.
...
- [x] Integrate Ready, calibration, active race, pause, results, semantic controls, avatar omission, and accessible status into the shared body playfield.
...
1. Games exposes Draw, Bubbles, Racing, and Return through one compact left column in landscape, with no hidden prior action.
2. Racing lazy-loads one pinned Phaser Canvas runtime only after Racing mounts; phone setup, Draw, and Bubbles do not load it.
...
6. One-player Racing supports landscape and reports active elapsed finish time. Two-player Racing requires landscape, renders left/right chase views with the other car visible inside each forward frustum, and declares a winner or exact tie.
...
11. Page suspension freezes active simulation; portrait ends and releases the run. User pause requires explicit Resume.
