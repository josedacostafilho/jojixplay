...
JojixPlay provides immediate procedural sound feedback throughout the rendered experience. Sound originates on the phone; the operating system routes it during external mirroring. [ADR-0020](../decisions/0020-app-owned-procedural-audio.md) governs the boundary.

## Ownership and graph

```text
trusted phone Start activation
        ↓
...
- **Start playing** constructs/resumes audio in the same trusted handler that requests camera, fullscreen, and wake behavior. Failure stops partial startup and returns to an actionable setup error.
- Audio is required on the phone.
...
- Stop, portrait rotation, and page teardown stop voices, remove listeners, disconnect nodes, and close the owned context.
...
- [x] Integrate trusted audio startup and complete cleanup into phone lifecycle ownership.
...
- [x] Keep Phaser audio disabled and pure sessions, pose packets, and deterministic simulation free of Web Audio state.
...
1. The phone rejects browsers without standard Web Audio support.
...
3. Game sound originates only on the phone.
