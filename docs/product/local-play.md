...
4. Press **Start local play** once to request camera permission and best-effort fullscreen/wake behavior.
...
- Portrait/landscape support is unchanged: Main Menu, Games, Draw, and one-player Bubbles/Racing accept either layout; two-player Bubbles/Racing requires landscape.
...
Local play requires the union of phone inference and playfield rendering capabilities: secure-context camera access, Screen Orientation type/angle, WebAssembly, WebGL 2 for MediaPipe, module workers, `createImageBitmap`, `requestVideoFrameCallback`, ResizeObserver, and Canvas 2D. It does not require Web Crypto, WebSockets, or WebRTC.

- Missing required capabilities produce one blocking, actionable unsupported-device panel.
- Camera denial, absence, busy state, orientation failure, or model failure returns to a retryable setup state with the owned camera/worker/wake resources stopped.
...
- Page unmount, explicit stop, and navigation release the camera tracks, frame callback, worker, pending layout request, wake lock, and local play state. Racing teardown remains owned by `BodyPlayfield`.
...
3. Local play asks for camera access only after **Start local play** and uses the user-facing camera through the existing 30 FPS-ceiling, single-flight inference path.
...
9. Fullscreen and wake lock are best-effort; rejection or absence does not block camera/game startup, while stop/unmount releases every acquired resource.
10. Automated production-browser coverage reaches a real first MediaPipe packet with fake camera input, and real-device acceptance records sustained phone performance, thermals, framing, orientation, and external-mirroring latency.
