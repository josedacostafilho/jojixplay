...
- Temporal processing remains consumer-specific: Draw path smoothing, two-hand grip evidence, Bubbles swept collision, Racing torso-lean hysteresis/response, button controls, and avatar presentation do not mutate or replace the canonical packet or consume one another's derived values.
...
- Starting Racing suspends control activation, calibrates the aspect-corrected shoulder/hip torso angle for three seconds of fresh input, then maps `8°` lean to discrete full steering with a `3°` release threshold and runs automatic throttle through a deterministic `60 Hz` fixed-step simulation. Hands affect Racing only through a one-second two-hands-overhead pause hold. The procedural avatar and ordinary pose cursor remain absent while Racing.
...
16. Racing torso-neutral calibration, coarse lean hysteresis, steering response, automatic throttle, continuous near-road projection, one-/two-player results, lazy forced-Canvas runtime, pause/recovery, and teardown satisfy [Racing game](racing-game.md).
...
- [x] Add lazy forced-Canvas Phaser Racing with pure input/simulation/projection modules, calibrated coarse torso-lean steering, automatic throttle, continuous near-road coverage, solo timing, landscape split-screen competition, pause controls, and complete lifecycle cleanup.
