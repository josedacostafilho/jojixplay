...
| --- | --- | --- |
| `apps/jojixplay` | Preact UI, permission, camera, worker, observation adapter, immersive lifecycle | SDK, Desenhar, Corrida, Swinging, Preact, MediaPipe |
...
Root `dist/` is the deployable app. `games/desenhar/dist/`, `games/corrida/dist/` and `games/swinging/dist/` are separate development artifacts and are not deployed. GitHub Actions validates before publishing `main`. See [ADR-0022](../decisions/0022-independent-3d-platform.md).
