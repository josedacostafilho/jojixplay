# Documentation

Current truth lives in these documents. Historical decisions may describe retired behavior.

- [Agent rules](../AGENTS.md)
- [Project status](project/status.md)
- [Next decisions](project/backlog.md)
- [Stack and commands](architecture/stack.md)
- [Architecture and workspace boundaries](architecture/overview.md)
- [Phone playroom](product/local-play.md)
- [Desenhar](product/desenhar.md)
- [Camera coordinates](product/camera-orientation.md)
- [Tracking quality and device acceptance](engineering/pose-quality.md)
- [Engineering standards](engineering/standards.md)
- [Testing](engineering/testing.md)
- [Workflow](engineering/workflow.md)
- [Decisions](decisions/README.md)

Update the canonical owner of a fact in the same change. Use repository-relative links, explicit status and ISO dates. Never describe planned capabilities as implemented.
