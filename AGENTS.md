...
As last verified on **2026-09-24**, JojixPlay runs entirely on a landscape phone. Operating-system or wired screen mirroring is the only television experience. One hidden camera and MediaPipe worker feed validated landscape packets directly into the mirrored `BodyPlayfield`; Draw and Bubbles use Canvas, Racing lazy-loads forced-Canvas Phaser around pure TypeScript simulation, and one phone-owned Web Audio runtime supplies sound. Every run starts with one-player inference; direct reconfiguration selects two players before updating display. Portrait gates entry and unmounts active play, releasing resources. Native landscape locking, fullscreen, and wake lock are attempted where supported. Real-device performance, thermals, camera metadata, and mirroring acceptance remain outstanding.
The canonical product contracts are [Play on this phone](docs/product/local-play.md), [Camera orientation and game layouts](docs/product/camera-orientation.md), [Procedural body avatar](docs/product/avatar-renderer.md), [Draw game](docs/product/draw-game.md), [Bubbles game](docs/product/bubbles-game.md), [Racing game](docs/product/racing-game.md), and [Application audio](docs/product/audio.md). The live implementation state is maintained in [Project status](docs/project/status.md), and exact tools and commands live in [Stack](docs/architecture/stack.md). Never infer capabilities beyond those sources.
...
The selected client stack is TypeScript, Vite, Preact, MediaPipe Tasks Vision, Canvas 2D, Phaser Canvas for Racing, native Web Audio, Node 24, and npm. GitHub Actions validates pull requests and deploys validated `main` artifacts to GitHub Pages. [ADR-0021](docs/decisions/0021-landscape-phone-only.md) governs the phone-only landscape topology; the [ADR index](docs/decisions/README.md) identifies active game, rendering, worker, audio, and toolchain decisions that govern these boundaries.
...
6. Long-lived secrets and personal data are never committed, logged, embedded in fixtures, or exposed in build artifacts. Camera pixels and pose landmarks stay on the phone and are never transmitted, persisted, or logged.
...
13. Every run starts with one-pose inference. An absolute switch to one or two poses is strictly applied by the phone controller; display changes occur only after in-memory reconfiguration succeeds.
14. Temporal processing never mutates `PosePacket` or creates one universal smoothed pose. Continuous drawing, two-hand grip classification, Bubbles point/swept collision, Racing steering, button controls, and presentation own explicitly separate derived signals with measured latency contracts; measurement must never log, persist, or transmit coordinates.
15. Every actionable view uses a frozen compact left column. Targets stay inside the projected landscape camera frame and rendering/hit testing share rectangles. Bubbles Starting/Playing and Racing Calibrating/Driving expose no targets; actionable message panels stay below the target layer.
...
23. `PosePacket.frame` is exactly `{ width, height, layout, epoch }`. Layout is `landscape`, width exceeds height, and epoch changes on committed source-basis changes. Every temporal consumer resets on epoch change. The phone owns quarter-turn normalization before the packet boundary; portrait source bitmaps may be normalized, but portrait canonical packets are rejected.
24. Setup and all games require landscape. Native landscape lock is attempted after trusted Start/fullscreen entry, but browser rejection never permits portrait play. Portrait unmounts the run and releases every resource; a new landscape entry starts a fresh run. No per-game layout policy, layout negotiation, portrait geometry, or layout restoration path exists.
...
26. The phone owns exactly one standard `AudioContext`, created or resumed synchronously from Start before the playfield becomes active. `BodyPlayfield` emits bounded semantic cues and continuous Draw/Racing state through the injected audio boundary; pure sessions, pose packets, and Phaser remain audio-free. Master mute is page-session-local. Cleanup closes every owned node, listener, voice, and context; no alternate sound path exists.
27. The root website is the only application entry. Queries and fragments fail clearly. Validated packets flow directly from camera lifecycle to `BodyPlayfield`; no pairing, peer transport, or camera preview exists. Packets expire after one second without updates. Stop, portrait, navigation, and unmount release camera, worker, pending work, game runtime, audio, wake lock, and owned fullscreen/orientation lock.
