...
As last verified on **2026-08-18**, JojixPlay is a greenfield static web application with three application modes and three shared playfield-local games implemented. In the paired topology, pose estimation remains on the phone, public decentralized rendezvous establishes a direct WebRTC connection, and only validated pose landmarks reach the television. In **Play on this phone**, the same camera controller feeds the same mirrored `BodyPlayfield` directly in memory with no pairing or peer transport; the required capture video stays visually hidden and no camera preview is rendered. The phone accepts portrait or landscape capture, normalizes MediaPipe input and landmarks into one upright coordinate basis, and publishes an explicit frame layout and epoch. The shared playfield presents detections through the canonical faceless procedural body avatar except inside Racing, grants temporary gesture-claimed menu control, and hosts ephemeral Draw, Bubbles, and Racing experiences. Draw and Bubbles use focused Canvas renderers; Racing lazy-loads one forced-Canvas Phaser runtime around a pure TypeScript fixed-step simulation, a denser deterministic course, analog calibrated torso steering, explicit opponent projection, and pseudo-3D road drawing. One native procedural Web Audio runtime belongs to the rendering host, starts only from the trusted TV/local activation, and remains absent from the paired camera phone. New play runs use one-player inference by default; paired acknowledged commands or direct local apply-then-display commands can select two-player inference or request an absolute camera layout without restarting capture. One-player games accept either layout; two-player Bubbles and Racing require landscape. Complete real-device acceptance, including all-in-one phone thermals/external-mirroring latency, sound, and Racing performance/steering ergonomics, remains outstanding; publication state is tracked separately in project status.

The canonical product contracts are [Phone-to-television prototype](docs/product/skeleton-viewer.md), [Play on this phone](docs/product/local-play.md), [Camera orientation and game layouts](docs/product/camera-orientation.md), [Procedural body avatar](docs/product/avatar-renderer.md), [Draw game](docs/product/draw-game.md), [Bubbles game](docs/product/bubbles-game.md), [Racing game](docs/product/racing-game.md), and [Application audio](docs/product/audio.md). The live implementation state is maintained in [Project status](docs/project/status.md), and exact tools and commands live in [Stack](docs/architecture/stack.md). Never infer capabilities beyond those sources.
...
| All-in-one phone journey, privacy, and lifecycle | `docs/product/local-play.md` |
...
- Do not introduce an application backend, persistence, TURN, alternate signaling path, second game engine, or alternate Racing renderer without an accepted replacement decision.
...
11. `PosePacket` remains unsmoothed, unmirrored pose data in the phone's upright canonical camera coordinates. Phone-owned quarter-turn normalization occurs before the packet boundary; avatar drawing, playfield mirroring, Draw, Bubbles, Racing input, cursor projection, adaptive layout, and hit testing never mutate packet data or swap anatomical landmark indices.
12. Pose-array position is never a player identity. Only bounded playfield-local nearest-torso continuity may preserve a temporary menu-control or Racing driver lease; it must not become a transmitted, persisted, or stable person identifier.
13. Every paired session and every local-play run starts with a one-pose inference limit. A switch to one or two poses is absolute and strictly applied by the phone controller; paired display changes require the exact acknowledgement, while local display changes occur only after the same in-memory reconfiguration succeeds.
14. Temporal processing never mutates `PosePacket` or creates one universal smoothed pose. Continuous drawing, two-hand grip classification, Bubbles point/swept collision, Racing steering, button controls, and presentation own explicitly separate derived signals with measured latency contracts; local diagnostics remain bounded aggregates and never log, persist, or transmit coordinates.
15. Main Menu targets form one frozen above-head row in portrait and one frozen compact left column in landscape. Settings, Games, Draw, actionable Bubbles phases, and actionable Racing phases always use the compact left column. Every target remains inside the projected canonical frame; hit testing and rendering share the same rectangles. Bubbles Starting/Playing and Racing Calibrating/Driving expose no action targets. Bubbles and Racing actionable message panels stay near the top and below the target layer; informational chrome must never cover a reachable button.
...
18. Main Menu, Settings, Games, Draw, and each actionable Bubbles or Racing phase replace the complete typed action set atomically. Main Menu is exactly Games/Players/Settings; Background exists only inside Settings beside Sound and Return. A transition retains the short-lived controller lease when possible but clears target hover/dwell/latching and requires neutral re-arming; deleted placeholder actions and effects must not return. Active Bubbles rounds and active/calibrating Racing suspend rather than merely hide body-control activation.
19. Draw paths remain ephemeral playfield-local normalized canonical-camera coordinates. Mirroring is presentation-only; only the leased pose's selected complete coarse hand supplies Pencil or Eraser points. A fresh `≤ 0.75 × shoulder span` two-hand grip engages immediately, remains active through the hysteresis band, and releases at `≥ 1.25 × shoulder span`; stale input, controller/view loss, or camera-basis changes cancel it. Supporting-hand loss alone neither releases the grip nor breaks the main-hand path; main-hand loss, bounds, toolbar entry, and implausible jumps break paths without adding bridge segments.
20. Bubbles state is ephemeral and playfield-local. Its player count is captured from the successfully applied mode on game entry; one-player pops use the sole right score slot, while two-player pops belong only to mirrored left/right screen slots selected from current torso position. Pose-array order, the menu lease, and any stable or transmitted identity must never own a Bubbles score.
...
24. Main Menu, Games, Draw, and one-player Bubbles/Racing support portrait and landscape; two-player Bubbles/Racing requires landscape. A game captures its entering layout. Incompatible entry uses one strict absolute request: the paired path requires exact acknowledgement, and local play waits for the same camera controller to commit the layout directly. An active mismatch withholds pose input and requests return to the captured layout. Draw retains art but cancels grip/path; Bubbles and Racing freeze and later resume their complete timed state. No aspect-ratio proxy, hot game reflow, screen-lock dependency, or orientation fallback exists.
25. Racing is one lazy `phaser@4.2.1` forced-Canvas path with no `AUTO`, WebGL, renderer fallback, or second engine. Pure TypeScript owns aspect-corrected shoulder/hip torso lean, three-second natural-stance calibration, a symmetric `3°` dead zone → `15°` full-scale smoothstep steering map, `80 ms` response, `150 ms` dropout grace, temporary driver leases, the one denser deterministic course, course/object projection, and `60 Hz` fixed-step rules. Hands affect Racing only through the one-second two-hands-overhead pause gesture. Projection always inserts an explicit near-road sample past the viewport bottom, paints a below-horizon terrain base, and renders the other car in each two-player chase view whenever it remains inside that camera's forward frustum; fixed segment alignment must never expose the blue canvas near the car. Phaser owns only the mounted playfield canvas, cameras, animation callback, drawing, resize, and teardown. One-player reports elapsed active time; two-player uses landscape split screen and ends on the first finish.
26. Television and local play each own exactly one standard `AudioContext`, created or resumed synchronously from their existing trusted Start action before the rendering session becomes active. The paired camera phone is silent. `BodyPlayfield` emits bounded semantic cues and continuous Draw/Racing state through the injected audio boundary; pure sessions, pose packets, transport, and Phaser remain audio-free. One master mute is page-session-local and untransmitted. There is no HTML Audio, prefixed API, audio dependency, remote asset, persistence, or alternate sound path; cleanup closes every owned node, listener, voice, and context.
26. Application routing accepts only one exact `mode=tv|phone|local` query. Local play feeds validated packets directly from the shared camera lifecycle to `BodyPlayfield`, creates no pairing or peer transport, renders no camera preview, withholds packets after one second without an update, and treats fullscreen/Screen Wake Lock as best-effort. Explicit stop, navigation, or unmount must release camera, worker, pending requests, game runtime, wake lock, and owned fullscreen state.
