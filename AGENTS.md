...
As last verified on **2026-08-14**, JojixPlay is a greenfield static web application with its first vertical slice and first game deployed to GitHub Pages. The product is a phone-to-television body-control playground: pose estimation remains on the phone, public decentralized rendezvous establishes a direct WebRTC connection, and only validated pose landmarks reach the television. The television mirrors presentation, grants a temporary gesture-claimed control lease, exposes body-operated Main Menu/Games navigation, and hosts the ephemeral Draw game. New sessions use one-player inference by default; an acknowledged television-to-phone command can select two-player inference without restarting the camera. Draw uses an immediate body-relative two-hand grip with wide release hysteresis, one main-hand Pencil/Eraser cursor, and a compact left-edge toolbar; the phone exposes bounded local pose diagnostics for real-device model decisions. Complete real-device acceptance remains outstanding.
...
14. Temporal processing never mutates `PosePacket` or creates one universal smoothed pose. Continuous drawing, two-hand grip classification, button controls, and presentation own explicitly separate derived signals with measured latency contracts; local diagnostics remain bounded aggregates and never log, persist, or transmit coordinates.
15. Main Menu and Games targets form one frozen row entirely above the controlling pose's highest visible face landmark. Draw targets form one smaller frozen vertical column just inside the projected camera frame's left edge and centered around the leased torso. Hit testing and rendering share the same rectangles; no alternate placement fallback exists.
...
18. Main Menu, Games, and Draw each replace the complete typed action set atomically. A transition retains the short-lived controller lease but clears target hover/dwell/latching and requires neutral re-arming; the deleted Circles action and effect must not return.
