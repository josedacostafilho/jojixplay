...
25. Racing is one lazy `phaser@4.2.1` forced-Canvas path with no `AUTO`, WebGL, renderer fallback, or second engine. Pure TypeScript owns calibrated two-hand steering, temporary driver leases, deterministic course projection, and `60 Hz` fixed-step rules; Phaser owns only the mounted television canvas, cameras, animation callback, drawing, resize, and teardown. One-player reports elapsed active time; two-player uses landscape split screen and ends on the first finish.
