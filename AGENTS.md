...
As last verified on **2026-08-15**, JojixPlay is a greenfield static web application with its first vertical slice and three television-local games implemented. The product is a phone-to-television body-control playground: pose estimation remains on the phone, public decentralized rendezvous establishes a direct WebRTC connection, and only validated pose landmarks reach the television. The phone accepts portrait or landscape capture, normalizes MediaPipe input and landmarks into one upright coordinate basis, and publishes an explicit frame layout and epoch. Phone and television present detections through the canonical faceless procedural body avatar except inside Racing, where the avatar is deliberately absent. The television mirrors presentation horizontally, grants temporary gesture-claimed menu control, exposes body-operated Main Menu/Games navigation, and hosts ephemeral Draw, Bubbles, and Racing experiences. Draw and Bubbles use focused Canvas renderers; Racing lazy-loads one forced-Canvas Phaser runtime around a pure TypeScript fixed-step simulation and pseudo-3D road projection. New sessions use one-player inference by default; acknowledged television-to-phone commands can select two-player inference or request an absolute camera layout without restarting the camera. Draw uses an immediate two-hand grip, Bubbles uses both complete hands for timed target collisions, and Racing uses calibrated coarse torso lean with automatic throttle. One-player games accept either layout; two-player Bubbles and Racing require landscape. Complete real-device acceptance, including Racing performance and steering ergonomics, remains outstanding; publication state is tracked separately in project status.
...
11. `PosePacket` remains unsmoothed, unmirrored pose data in the phone's upright canonical camera coordinates. Phone-owned quarter-turn normalization occurs before the packet boundary; avatar drawing, television mirroring, Draw, Bubbles, Racing input, cursor projection, adaptive layout, and hit testing never mutate transport data or swap anatomical landmark indices.
12. Pose-array position is never a player identity. Only bounded television-local nearest-torso continuity may preserve a temporary menu-control or Racing driver lease; it must not become a transmitted, persisted, or stable person identifier.
...
24. Main Menu, Games, Draw, and one-player Bubbles/Racing support portrait and landscape; two-player Bubbles/Racing requires landscape. A game captures its entering layout. Incompatible entry uses one strict acknowledged TV-to-phone absolute layout request, while an active mismatch withholds pose input and requests return to the captured layout. Draw retains art but cancels grip/path; Bubbles and Racing freeze and later resume their complete timed state. No aspect-ratio proxy, hot game reflow, screen-lock dependency, or orientation fallback exists.
25. Racing is one lazy `phaser@4.2.1` forced-Canvas path with no `AUTO`, WebGL, renderer fallback, or second engine. Pure TypeScript owns aspect-corrected shoulder/hip torso lean, three-second natural-stance calibration, discrete `8°` enter/`3°` release steering hysteresis, `80 ms` response, `150 ms` dropout grace, temporary driver leases, deterministic course projection, and `60 Hz` fixed-step rules. Hands affect Racing only through the one-second two-hands-overhead pause gesture. Projection always inserts an explicit near-road sample past the viewport bottom and paints a below-horizon terrain base; fixed segment alignment must never expose the blue canvas near the car. Phaser owns only the mounted television canvas, cameras, animation callback, drawing, resize, and teardown. One-player reports elapsed active time; two-player uses landscape split screen and ends on the first finish.
