...
As last verified on **2026-08-14**, JojixPlay is a greenfield static web application with its first vertical slice and two television-local games implemented. The product is a phone-to-television body-control playground: pose estimation remains on the phone, public decentralized rendezvous establishes a direct WebRTC connection, and only validated pose landmarks reach the television. Phone and television present each detection as the one canonical faceless procedural body avatar; its private one-player display stabilization never changes control or game input. The television mirrors presentation, grants a temporary gesture-claimed control lease, exposes body-operated Main Menu/Games navigation, and hosts the ephemeral Draw and Bubbles games behind separate Canvas renderers. New sessions use one-player inference by default; an acknowledged television-to-phone command can select two-player inference without restarting the camera. Draw uses an immediate body-relative two-hand grip with wide release hysteresis and one main-hand Pencil/Eraser cursor. Bubbles uses both complete hands independently of the menu lease, exact timed rounds, and identity-independent screen-side scoring. The phone exposes bounded local pose diagnostics for real-device model decisions. Complete real-device acceptance remains outstanding; publication state is tracked separately in project status.

The canonical product contracts are [Phone-to-television prototype](docs/product/skeleton-viewer.md), [Procedural body avatar](docs/product/avatar-renderer.md), [Draw game](docs/product/draw-game.md), and [Bubbles game](docs/product/bubbles-game.md). The live implementation state is maintained in [Project status](docs/project/status.md), and exact tools and commands live in [Stack](docs/architecture/stack.md). Never infer capabilities beyond those sources.
...
11. `PosePacket` remains in raw phone-camera coordinates. Avatar drawing, television mirroring, Draw, Bubbles simulation/input, cursor projection, adaptive layout, and hit testing share one projection contract and never mutate transport data or swap anatomical landmark indices.
...
15. Main Menu and Games targets form one frozen row entirely above the controlling pose's highest visible face landmark. Draw and Bubbles Ready/Finished targets form one smaller frozen vertical column just inside the projected camera frame's left edge and centered around the leased torso. Hit testing and rendering share the same rectangles; no alternate placement fallback exists. Bubbles Starting/Playing exposes no action targets.
...
19. Draw paths remain ephemeral television-local normalized raw-camera coordinates. Mirroring is presentation-only; only the leased pose's selected complete coarse hand supplies Pencil or Eraser points. A fresh `≤ 0.75 × shoulder span` two-hand grip engages immediately, remains active through the hysteresis band, and releases at `≥ 1.25 × shoulder span`; stale input, controller/view loss, or frame changes cancel it. Supporting-hand loss alone neither releases the grip nor breaks the main-hand path; main-hand loss, bounds, toolbar entry, and implausible jumps break paths without adding bridge segments.
...
21. Bubbles uses exact monotonic three-second/60-second deadlines, both complete coarse hands, current-point plus bounded fresh swept collision, and radius-aware clamp-and-reflect inside the projected camera arena. A bubble scores at most once; pose loss never pauses the clock; non-increasing, stale, missing, out-of-frame, frame-changed, or implausibly long hand motion cannot create a bridge collision.
